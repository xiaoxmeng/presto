...
std::shared_ptr<connector::ColumnHandle> toColumnHandle(
  ...
  facebook::velox::dwio::type::fbhive::HiveTypeParser hiveTypeParser;
  if (auto hiveColumn =
...
std::vector<std::string> fromStringList(
    const protocol::List<protocol::String>& stringList) {
  std::vector<std::string> stringVector;
  stringVector.reserve(stringList.size());
...
std::shared_ptr<HiveBucketProperty> toHiveBucketProperty(
  ...
  VELOX_USER_CHECK(
      ...
      "Bucket table sort write is not supported: {}",
      toJsonString(*bucketProperty));

  const int32_t bucketCount = bucketProperty->bucketCount;
  if (bucketCount <= 0) {
    return nullptr;
  }

  const std::vector<std::string> bucketedBy =
      fromStringList(bucketProperty->bucketedBy);
  VELOX_USER_CHECK(
      !bucketedBy.empty(),
      "Bucketed by columns hasn't been set: {}",
      toJsonString(*bucketProperty));
  ...
  if (kind == HiveBucketProperty::Kind::kHiveCompatible) {
    VELOX_USER_CHECK_NULL(
        ...
        "Unexpected bucketed by types: {}",
        toJsonString(*bucketProperty));
    bucketedTypes.reserve(bucketedBy.size());
    for (const auto& bucketedColumn : bucketedBy) {
      ...
      if (bucketedType == nullptr) {
        VELOX_FAIL("bucketed by column \"{}\" not found", bucketedColumn);
      }
    ...
    VELOX_USER_CHECK_EQ(
        ...
        bucketedBy.size(),
        "Bucketed by types is not set properly: {}",
        toJsonString(*bucketProperty));
    bucketedTypes = stringToTypes(bucketProperty->types);
  }
  auto sortedBy = toHiveSortingColumns(bucketProperty->sortedBy);
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (auto createHandle = std::dynamic_pointer_cast<protocol::CreateHandle>(
    ...
    VELOX_USER_CHECK(
        ...
        "Bucketed table must been partitioned: {}",
    ...
    VELOX_UNSUPPORTED(
        "Unsupported tabgle writer handle: {}",
